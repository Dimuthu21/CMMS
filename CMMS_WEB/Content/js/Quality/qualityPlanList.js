...
$(document).ready(function () {
    ...
    $(document).on(dataTableClickEvent, '#tblDetails tbody tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewQualityPlanUrl").val() + "/" + data.QualityPlanId, null, null);
    });
});

function ClearQualityPlanListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateQualityPlanListFilter() {
    var result = true;
    return result;
}

function FilterQualityPlanList() {
    if (ValidateQualityPlanListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseQualityPlanListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleQualityPlanListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleQualityPlanListLoadFailure(null)
...
function handleQualityPlanListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading quality plan list.", null, null, null)
}
