...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewTestMethodUrl").val() + "/" + data.TestMethodId, null, null);
    });
});

function ClearTestMethodListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateTestMethodListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterTestMethodList() {
    if (ValidateTestMethodListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseTestMethodListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleTestMethodListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleTestMethodListLoadFailure(null)
...
function handleTestMethodListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading test method list.", null, null, null)
}
