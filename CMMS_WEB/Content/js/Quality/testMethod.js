const navigationMenuName = "TestMethod";

const testMethodId = $("#TestMethodId").val();
...
function SaveTestMethod() {
    ...
    if (ValidateTestMethod()) {

        showProgressLoading(null, 'Saving test method. Please wait...');

        $.ajax({
            url: $("#TestMethodSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnTestMethod(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(testMethodId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(testMethodId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'test method');
}
...
function ProceedDeleteEntry() {
    if (testMethodId != null) {
        ...
        showProgressLoading(null, 'Deleting test method. Please wait...');

        $.ajax({
            url: $("#TestMethodDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#TestMethodListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
