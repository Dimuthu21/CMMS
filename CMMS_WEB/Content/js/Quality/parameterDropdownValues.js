const navigationMenuName = "ParameterDropdownValues";

const parameterDropdownValueId = $("#ParameterDropdownValueId").val();
...
function ValidateParameterDropdownValue() {
    var result = true;
...
function ReturnParameterDropdownValue() {
    var code = $("#Code").val();
    ...
    return {
        ...
        'ParameterDropdownValueId': parameterDropdownValueId,
        'Code': code,
...
function SaveParameterDropdownValue() {
    ChangeSaveButtonState(true);

    if (ValidateParameterDropdownValue()) {

        showProgressLoading(null, 'Saving parameter dropdown value. Please wait...');

        $.ajax({
            url: $("#ParameterDropdownValueSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnParameterDropdownValue(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(parameterDropdownValueId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(parameterDropdownValueId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'parameter dropdown value');
}

function ProceedDeleteEntry() {
    if (parameterDropdownValueId != null) {
        ChangeDeleteButtonState(true);

        showProgressLoading(null, 'Deleting parameter dropdown value. Please wait...');

        $.ajax({
            url: $("#ParameterDropdownValueDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#ParameterDropdownValueListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
