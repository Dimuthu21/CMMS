...
$(document).ready(function () {
    FilterParameterDropdownValueList();

    // Use delegated handlers on tbody so they work after DataTable redraws
    $('#tblDetails tbody').on('click', '.details-control', function (e) {
    ...
    $('#tblDetails tbody').on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewParameterDropdownValueUrl").val() + "/" + data.ParameterDropdownValueId, null, null);
    });
});

function ClearParameterDropdownValueListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateParameterDropdownValueListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterParameterDropdownValueList() {
    if (ValidateParameterDropdownValueListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseParameterDropdownValueListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleParameterDropdownValueListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleParameterDropdownValueListLoadFailure(null)
...
function handleParameterDropdownValueListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading parameter dropdown value list.", null, null, null)
}
