...
const uomGroupId = $("#UOMGroupId").val();
...
function SaveUOMGroup() {
    ChangeSaveButtonState(true);

    if (ValidateUOMGroup()) {

        showProgressLoading(null, 'Saving UOM group. Please wait...');

        $.ajax({
            url: $("#UOMGroupSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnUOMGroup(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(uomGroupId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(uomGroupId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'uom group');
}
...
function ProceedDeleteEntry() {
    if (uomGroupId != null) {
        ...
        showProgressLoading(null, 'Deleting uom group. Please wait...');

        $.ajax({
            url: $("#UOMGroupDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#UOMGroupListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
