const navigationMenuName = "Trustee";
...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewTrusteeUrl").val() + "/" + data.TrusteeId, null, null);
    });
});

function ClearTrusteeListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateTrusteeListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterTrusteeList() {
    if (ValidateTrusteeListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleTrusteeListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleTrusteeListLoadFailure(null)
...
function handleTrusteeListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading trustee list.", null, null, null)
}
