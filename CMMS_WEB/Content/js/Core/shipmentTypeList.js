...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewShipmentTypeUrl").val() + "/" + data.ShipmentTypeId, null, null);
    });
});

function ClearShipmentTypeListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateShipmentTypeListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterShipmentTypeList() {
    if (ValidateShipmentTypeListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseShipmentTypeListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleShipmentTypeListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleShipmentTypeListLoadFailure(null)
...
function handleShipmentTypeListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading shipment type list.", null, null, null)
}
