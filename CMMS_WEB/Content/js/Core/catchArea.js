const navigationMenuName = "CatchArea";

const catchAreaId = $("#CatchAreaId").val();
...
function SaveCatchArea() {
    ChangeSaveButtonState(true);

    if (ValidateCatchArea()) {

        showProgressLoading(null, 'Saving catch area. Please wait...');

        $.ajax({
            url: $("#CatchAreaSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnCatchArea(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(catchAreaId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(catchAreaId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'catch area');
}
...
function ProceedDeleteEntry() {
    if (catchAreaId != null) {
        ...
        showProgressLoading(null, 'Deleting catch area. Please wait...');

        $.ajax({
            url: $("#CatchAreaDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#CatchAreaListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
