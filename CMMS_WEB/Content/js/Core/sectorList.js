...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewSectorUrl").val() + "/" + data.SectorId, null, null);
    });
});

function ClearSectorListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateSectorListFilter() {
    var result = true;
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleSectorListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleSectorListLoadFailure(null)
...
function handleSectorListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading sector list.", null, null, null)
}
