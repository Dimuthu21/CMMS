const navigationMenuName = "ShipmentType";

const shipmentTypeId = $("#ShipmentTypeId").val();
...
function SaveShipmentType() {
    ...
    if (ValidateShipmentType()) {

        showProgressLoading(null, 'Saving shipment type. Please wait...');

        $.ajax({
            url: $("#ShipmentTypeSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnShipmentType(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(shipmentTypeId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(shipmentTypeId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'shipment type');
}
...
function ProceedDeleteEntry() {
    if (shipmentTypeId != null) {
        ...
        showProgressLoading(null, 'Deleting shipment type. Please wait...');

        $.ajax({
            url: $("#ShipmentTypeDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#ShipmentTypeListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
