...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewCatchingMethodUrl").val() + "/" + data.CatchingMethodId, null, null);
    });
});

function ClearCatchingMethodListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateCatchingMethodListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterCatchingMethodList() {
    if (ValidateCatchingMethodListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseCatchingMethodListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleCatchingMethodListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleCatchingMethodListLoadFailure(null)
...
function handleCatchingMethodListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading catching method list.", null, null, null)
}
