...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewLanguagesUrl").val() + "/" + data.LanguagesId, null, null);
    });
});

function ClearLanguagesListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateLanguagesListFilter() {
    var result = true;
...
function FilterLanguagesList() {
    if (ValidateLanguagesListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseLanguagesListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleLanguagesListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleLanguagesListLoadFailure(null)
...
function handleLanguagesListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading languages list.", null, null, null)
}
