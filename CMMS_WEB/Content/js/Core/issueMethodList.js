...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewIssueMethodUrl").val() + "/" + data.IssueMethodId, null, null);
    });
});

function ClearIssueMethodListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateIssueMethodListFilter() {
    var result = true;
...
function FilterIssueMethodList() {

    if (ValidateIssueMethodListFilter()) {
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                ajax =
                    $.ajax({
                        url: $("#BrowseIssueMethodListUrl").val(),
                        type: "POST",
                        ...
                        success: function (result) {
                            if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                                ...
                                handleIssueMethodListLoadFailure(null)
                            }
                        },
                        error: function (e) {
                            handleIssueMethodListLoadFailure(null)
...
function handleIssueMethodListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading issue method list.", null, null, null)
}
