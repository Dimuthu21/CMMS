const navigationMenuName = "CatchingMethod";

const catchingMethodId = $("#CatchingMethodId").val();
...
function ValidateCatchingMethod() {
    var result = true;
...
function ReturnCatchingMethod() {
    ...
    return {
        ...
        'CatchingMethodId': catchingMethodId,
        'Code': code,
...
function SaveCatchingMethod() {
    ChangeSaveButtonState(true);

    if (ValidateCatchingMethod()) {

        showProgressLoading(null, 'Saving catching method. Please wait...');

        $.ajax({
            url: $("#CatchingMethodSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnCatchingMethod(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(catchingMethodId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(catchingMethodId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'catching method');
}

function ProceedDeleteEntry() {
    if (catchingMethodId != null) {
        ChangeDeleteButtonState(true);

        showProgressLoading(null, 'Deleting catching method. Please wait...');

        $.ajax({
            url: $("#CatchingMethodDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#CatchingMethodListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
