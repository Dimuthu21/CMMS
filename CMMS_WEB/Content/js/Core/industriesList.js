...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewIndustryUrl").val() + "/" + data.IndustryId, null, null);
    });
});

function ClearIndustryListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateIndustryListFilter() {
    var result = true;
    ...
    if (ValidateIndustryListFilter()) {
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseIndustriesListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleIndustryListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleIndustryListLoadFailure(null)
...
function handleIndustryListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading industry list.", null, null, null)
}
