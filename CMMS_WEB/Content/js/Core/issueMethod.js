const navigationMenuName = "IssueMethod";

const issueMethodId = $("#IssueMethodId").val();
...
function SaveIssueMethod() {
    ChangeSaveButtonState(true);

    if (ValidateIssueMethod()) {

        showProgressLoading(null, 'Saving issue method. Please wait...');

        $.ajax({
            url: $("#IssueMethodSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnIssueMethod(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(issueMethodId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(issueMethodId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'issue method');
}

function ProceedDeleteEntry() {
    if (issueMethodId != null) {
        ChangeDeleteButtonState(true);

        showProgressLoading(null, 'Deleting issue method. Please wait...');

        $.ajax({
            url: $("#IssueMethodDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#IssueMethodListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
