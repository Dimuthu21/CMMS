...
function SaveTrustee() {
    ...
    if (ValidateTrustee()) {
        ...
        showProgressLoading(null, 'Saving trustee. Please wait...');

        $.ajax({
            url: $("#TrusteeSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnTrustee(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(trusteeId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(trusteeId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'trustee');
}
...
function ProceedDeleteEntry() {
    if (trusteeId != null) {
        ...
        showProgressLoading(null, 'Deleting trustee. Please wait...');

        $.ajax({
            url: $("#TrusteeDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#TrusteeListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
