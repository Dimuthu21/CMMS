...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewCatchAreaUrl").val() + "/" + data.CatchAreaId, null, null);
    });
});

function ClearCatchAreaListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateCatchAreaListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterCatchAreaList() {
    if (ValidateCatchAreaListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseCatchAreaListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleCatchAreaListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleCatchAreaListLoadFailure(null)
...
function handleCatchAreaListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading catch area list.", null, null, null)
}
