...
function SaveFAOType() {
    ChangeSaveButtonState(true);

    if (ValidateFAOType()) {

        showProgressLoading(null, 'Saving FAO type. Please wait...');

        $.ajax({
            url: $("#FAOTypeSaveUrl").val(),
            method: 'POST',
            dataType: 'JSON',
            data: ReturnFAOType(),
            beforeSend: function () {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    ...
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
                        DisplayUserActionSaveFailureAlert(faoTypeId, result.UserInformationMessageType, result.UserInformationMessageTitle, result.UserInformationMessageContent)
                    } else {
                        DisplayUserActionSaveFailureAlert(faoTypeId, null, null, null);
                    }
...
function ConfirmDeleteEntry() {
    DisplayConfirmationAlert(Alert_Action_Enum.Delete, Alert_Type_Enum.Warning, null, null, null, null, ProceedDeleteEntry, null, true, null, null, null, null, 'FAO type');
}
...
function ProceedDeleteEntry() {
    if (faoTypeId != null) {
        ...
        showProgressLoading(null, 'Deleting FAO type. Please wait...');

        $.ajax({
            url: $("#FAOTypeDeleteUrl").val(),
            type: "POST",
            dataType: "JSON",
            data: {
            ...
            success: function (result) {
                if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                    location.href = $("#FAOTypeListUrl").val();
                } else {
                    dismissProgressLoading();
                    if (Server_Error_Message_Display_Execution_Results.includes(result.ExecutionResultId)) {
