...
$(document).ready(function () {
    ...
    tblDetails.on(dataTableClickEvent, 'tr', function (e) {
        ...
        if (!data) return;
        OpenResourceUrl($("#ViewCountryofOriginUrl").val() + "/" + data.CountryofOriginId, null, null);
    });
});

function ClearCountryofOriginListFilter() {
    $("#Code").val('');
    $("#Name").val('');
...
function ValidateCountryofOriginListFilter() {
    var result = true;
    // list filter validations if exists any
    return result;
}

function FilterCountryofOriginList() {
    if (ValidateCountryofOriginListFilter()) {
        var currentFilter = {
            "Code": $("#Code").val(),
            "Name": $("#Name").val(),
        ...
        tblDetails = $("#tblDetails").DataTable({
            ...
            ajax: function (data, callback, s) {
                ...
                $.ajax({
                    url: $("#BrowseCountryofOriginListUrl").val(),
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.ExecutionResultId == Execution_Result_Enum.Success) {
                            ...
                            handleCountryofOriginListLoadFailure(null)
                        }
                    },
                    error: function (e) {
                        handleCountryofOriginListLoadFailure(null)
...
function handleCountryofOriginListLoadFailure(ex) {
    if (ex != null) console.log(ex);
    $('#tblDetailsOverlay').css('display', 'none');
    DisplayUserActionAlert(Alert_Action_Enum.View, Alert_Type_Enum.Danger, null, "An error occurred while loading country of origin list.", null, null, null)
}
